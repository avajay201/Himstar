...
import { fetchUserBank, deleteUserBank, saveUserBank, withdrawReqFetch, withdrawReqCreate , profile} from "../../actions/ApiActions";
...
const Wallet = ({ navigation }) => {
  ...
  const fetchWithdrawHistory = async()=>{
    setLoading(true);
    const result = await withdrawReqFetch(navigation);
    if (result[0] === 200){
      setWithdrawalRequests(result[1].withdrawal_history);
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={{
              ...
              <View>
                <Text style={{ fontSize: 16, fontWeight: "500", color: '#333' }}>₹{item.amount}</Text>
                <Text style={{ fontSize: 14, color: "#777" }}>{item.requested_at_formatted}</Text>
              </View>
              <Text
                style={{
