...
const Register = ({navigation}) => {
    ...
    switch (step) {
      case 1:
        if (!formData.username) {
          ...
          newErrors.username =
            'Username must be at least 6 characters long and start with a letter.';
        }
  ...
  const handleRegister = async () => {
    ...
    if (result[0] === 200) {
      ...
      ToastAndroid.show(
        typeof errorMsg == 'object' ? errorMsg[0] : errorMsg,
        ToastAndroid.SHORT,
      );
    }
    setLoading(false);
  ...
  const handleGoogleRegister = async () => {
    try {
      ...
      setLoading(true);
      const result = await userGoogleRegistration({
  ...
  const renderRadioButton = label => (
    <TouchableOpacity
      ...
      <View
        style={[
          styles.radioCircle,
          formData.gender === label.toLowerCase() && styles.selectedCircle,
        ]}
      />
          ...
          <View style={styles.container}>
            ...
            {step === 2 && (
              <>
                ...
                <TouchableOpacity
                  onPress={() => setDobPickerVisible(!dobPickerVisible)}>
                  <TextInput
                    style={[styles.input2, {pointerEvents: 'none'}]} // Add pointerEvents: 'none' to prevent link behavior
                    value={formData.dob}
                    editable={false}
                    maxLength={10}
                  />
              ...
              <>
                <TouchableOpacity
                  ...
                  <Text style={styles.linkText}>
                    <Text style={styles.linkBlackText}>
