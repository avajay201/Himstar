...
const ViewComp = ({route, navigation}) => {
  ...
  const compRegister = async () => {
    ...
    if (!email || !name || !phone || !reg_id) {
      ToastAndroid.show(
        'Please update your profile before competetion register.',
        ToastAndroid.SHORT,
      );
      return;
    }
    navigation.navigate('Payment', {
      ...
      compType: competition?.competition_type,
      amount: String(competition?.price),
    ...
    <ScrollView contentContainerStyle={styles.container}>
        ...
        source={{
          uri:
            competition?.competition_type === 'tournament'
              ? competition?.competitions?.banner_image &&
                competition?.competitions?.banner_image?.includes('media')
                ? BASE_URL + competition?.competitions?.banner_image
                : competition?.competitions?.file_uri
      ...
      <View style={styles.detailsContainer}>
        ...
        {competition?.reg_open && (
          <TouchableOpacity
            style={[styles.registerButton, {backgroundColor: '#B94EA0'}]}
            onPress={() =>
              competition?.is_done
                ? navigation.navigate('Leaderboard', {compId: competition?.id})
